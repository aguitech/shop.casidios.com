...
//$qry_sucursal = "select * from sucursal left join usuario_sucursal on usuario_sucursal.id_sucursal = sucursal.id_sucursal left join empresa on empresa.id_empresa = sucursal.id_empresa where sucursal = '{$exploded_url[2]}' and empresa = '{$exploded_url[1]}'";
//$qry_sucursal = "select * from sucursal left join usuario_sucursal on usuario_sucursal.id_sucursal = sucursal.id_sucursal left join empresa on empresa.id_empresa = sucursal.id_empresa where sucursal.sucursal = '{$exploded_url[2]}' and empresa.empresa = '{$exploded_url[1]}'";
...
//$mesa_value = $obj->get_row("select * from mesa where id_mesa = '{$exploded_url[3]}'");
//$qry_mesa = "select * from mesa where id_mesa = {$exploded_url[3]} and id_sucursal = $id_sucursal";
...
if($id_sucursal != "" && $id_usuario != ""):
    ...
    //print_r($mesa_value);
    /*
    if(isset($id_mesa) || !(empty($id_mesa) || $id_mesa != "" || $id_mesa != 0)){
        $_SESSION["mesa"] = $id_mesa . "sa";
        echo "que pasa";
        
    }else{
        $_SESSION["mesa"] = $id_mesa . "s";
...
<!--		<script src="js/jquery-1.7.2.js"></script>-->
	...
	<body onload="">
		...
		*/ ?>
		...
		<div style="" id="fondo_especial" onclick="$('#fondo_especial').slideUp('slow'); $('#banner_especial').hide('slow');">
			
		</div>
		<div style="" id="banner_especial" onclick="$('#fondo_especial').slideUp('slow'); $('#banner_especial').hide('slow');">
                                    ...
                                    <div>
            							<div class="contenedor_punto_venta">
                    						...
                    						<form id="" method="post" action="">
                                        		...
                                        		<div>
                                        			<!-- <input type="submit" />-->
                                        			<button class="btn waves-effect waves-light bg_aguitech" type="submit" name="action"><?php if(isset($resultado->id_venta) && $resultado->id_venta != ""): ?>ACTUALIZAR<?php else: ?>CREAR<?php endif; ?> <i class="material-icons right">send</i>add_shopping_cart</button>
                                        		</div>
                                        		*/ ?>
...
<!--							Desarrollado por AGUITECH &copy; <?php echo date("Y"); ?>-->
		...
		<div style="width:40px; height:40px; position:fixed; bottom:20px; left:0; background:gray; z-index:2; display:flex; align-items:center; justify-content:center; cursor:pointer; border-radius:0 7px 7px 0;" onclick="mostrar_pedidos('<?php echo md5($mesa_valor->id_mesa); ?>');">
