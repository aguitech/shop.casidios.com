...
//print_r($_SESSION);
//echo "as";
//echo $_SESSION["mesamd5"];
//exit();

//$mesa_md5 = $_SESSION["mesamd5"];
//echo "select * from mesa where md5(id_mesa) = '$mesa_md5'";
...
if(empty($_SESSION["cantidad_productos"])){
    ...
    */
    ...
    //$qry_add = "insert into venta (venta, id_sucursal, id_usuario, id_mesa) values ('{$val_venta}', {$id_sucursal}, {$id_usuario}, {$id_mesa})";
    //$qry_add = "insert into venta (venta, id_sucursal, id_usuario, id_mesa) values ('{$val_venta}', {$id_sucursal}, {$id_usuario}, {$id_mesa})";
    //$qry_add = "insert into venta (venta, id_sucursal, id_usuario, id_mesa) values ('{$val_venta}', {$id_sucursal}, {$id_usuario}, {$id_mesa})";
    //$qry_add = "insert into venta (venta, id_sucursal, id_usuario) values ('{$val_venta}', {$id_sucursal}, {$id_usuario})";
    //$id_ship
    //$qry_add = "insert into venta (venta, id_sucursal, id_usuario, id_mesa) values ('{$val_venta}', {$id_sucursal}, {$id_usuario}, {$id_mesa})";
    $qry_add = "insert into venta (venta, id_sucursal, id_usuario, id_mesa, id_registro, id_registro_direccion) values ('{$val_venta}', {$id_sucursal}, {$id_usuario}, {$id_mesa}, {$id_registro}, {$id_ship})";
    ...
    $venta_creada = $obj->get_row("select * from venta where (venta = '{$val_venta}' and id_sucursal = {$id_sucursal} and id_usuario = {$id_usuario} and id_registro = {$id_registro} and id_registro_direccion = $id_ship) order by id_venta desc");
    
    $id_venta_creada = $venta_creada->id_venta;
    
    //echo $id_venta_creada;
    //exit();
    //$qry_add_empresa = "insert into usuario_empresa (id_usuario, id_empresa) values ({$id_usuario}, {$id_empresa_creada})";
    //$obj->query($qry_add_empresa);
